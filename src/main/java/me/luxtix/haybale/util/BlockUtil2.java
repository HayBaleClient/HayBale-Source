...
public class BlockUtil2
    ...
    public static List<BlockPos> getBlockSphere(float breakRange, Class clazz) {
        ...
        positions.addAll(BlockUtil2.getSphere(EntityUtil.getPlayerPos(BlockUtil2.mc.player), breakRange, (int) breakRange, false, true, 0).stream().filter(pos -> clazz.isInstance(BlockUtil2.mc.world.getBlockState(pos).getBlock())).collect(Collectors.toList()));
        return positions;
    }
    ...
    public static List<EnumFacing> getPossibleSides(BlockPos pos) {
        ...
        for (EnumFacing side : EnumFacing.values()) {
            ...
            if (!BlockUtil2.mc.world.getBlockState(neighbour).getBlock().canCollideCheck(BlockUtil2.mc.world.getBlockState(neighbour), false) || (blockState = BlockUtil2.mc.world.getBlockState(neighbour)).getMaterial().isReplaceable())
                continue;
            facings.add(side);
        }
        return facings;
    ...
    public static void rightClickBlockLegit(BlockPos pos, float range, boolean rotate, EnumHand hand, AtomicDouble Yaw, AtomicDouble Pitch, AtomicBoolean rotating) {
        ...
        for (EnumFacing side : EnumFacing.values()) {
            ...
            BlockUtil2.mc.playerController.processRightClickBlock(BlockUtil2.mc.player, BlockUtil2.mc.world, pos, side, hitVec, hand);
    ...
    public static boolean placeBlock(BlockPos pos, EnumHand hand, boolean rotate, boolean packet, boolean isSneaking) {
        ...
        if (!BlockUtil2.mc.player.isSneaking() && (blackList.contains(neighbourBlock) || shulkerList.contains(neighbourBlock))) {
            BlockUtil2.mc.player.connection.sendPacket(new CPacketEntityAction(BlockUtil2.mc.player, CPacketEntityAction.Action.START_SNEAKING));
    ...
    public static void placeBlockStopSneaking(BlockPos pos, EnumHand hand, boolean rotate, boolean packet, boolean isSneaking) {
        ...
        if (!isSneaking && sneaking) {
            BlockUtil2.mc.player.connection.sendPacket(new CPacketEntityAction(BlockUtil2.mc.player, CPacketEntityAction.Action.STOP_SNEAKING));
        }
    }
    ...
    public static List<BlockPos> possiblePlacePositions(float placeRange) {
        NonNullList positions = NonNullList.create();
        positions.addAll(BlockUtil2.getSphere(EntityUtil.getPlayerPos(BlockUtil2.mc.player), placeRange, (int) placeRange, false, true, 0).stream().filter(BlockUtil2::canPlaceCrystal).collect(Collectors.toList()));
        return positions;
    }
    ...
    public static List<BlockPos> possiblePlacePositions(float placeRange, boolean specialEntityCheck) {
        NonNullList positions = NonNullList.create();
        positions.addAll(BlockUtil2.getSphere(EntityUtil.getPlayerPos(BlockUtil2.mc.player), placeRange, (int) placeRange, false, true, 0).stream().filter(pos -> BlockUtil2.canPlaceCrystal(pos, specialEntityCheck)).collect(Collectors.toList()));
        return positions;
    }
    ...
    public static boolean canPlaceCrystal(BlockPos blockPos, boolean specialEntityCheck) {
        block7:
        {
            ...
            try {
                ...
                if (BlockUtil2.mc.world.getBlockState(boost).getBlock() != Blocks.AIR || BlockUtil2.mc.world.getBlockState(boost2).getBlock() != Blocks.AIR) {
                    return false;
                }
                ...
                return BlockUtil2.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost)).isEmpty() && BlockUtil2.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost2)).isEmpty();
    ...
    public static boolean rayTracePlaceCheck(BlockPos pos, boolean shouldCheck, float height) {
        return !shouldCheck || BlockUtil2.mc.world.rayTraceBlocks(new Vec3d(BlockUtil2.mc.player.posX, BlockUtil2.mc.player.posY + (double) BlockUtil2.mc.player.getEyeHeight(), BlockUtil2.mc.player.posZ), new Vec3d(pos.getX(), (float) pos.getY() + height, pos.getZ()), false, true, false) == null;
    }
