...
public class SelfAnvil extends Module {
    ...
    public void onEnable() {
        ...
        if (InventoryUtil.findHotbarBlock(BlockAnvil.class) == -1 || PlayerUtil2.findObiInHotbar() == -1) {
            this.disable();
        }
    }
    ...
    public void onTick() {
        ...
        BlockPos anvilPos = EntityUtil2.getFlooredPos(target).up(2);
        if (BlockUtil2.canPlaceBlock(anvilPos)) {
            placeAnvil(anvilPos);
            placedAmmount++;
        } else {
    ...
    private void placeObi(BlockPos pos) {
        int old = mc.player.inventory.currentItem;
        this.switchToSlot(PlayerUtil2.findObiInHotbar());
        BlockUtil.placeBlock(pos, EnumHand.MAIN_HAND, rotate.getValue(), true, false);
        this.switchToSlot(old);
    }
